...
const mapStateToProps = (state) => ({
  totalItems: state.kitbag.market.losts.totalItems,
  items: state.kitbag.market.losts.items,
});
...
const KitbagMarketLostAnnouncement = ({
  group,
  items,
  totalItems,
  fetchKitbagMarketItems,
  ...
  useEffect(() => {
    fetchKitbagMarketItems(MarketTypes.LOST, 5);
  }, [fetchKitbagMarketItems]);
  ...
  return (
    <div className="card border-0">
      <div className="alert alert-danger mb-0" role="alert">
        <h2 className="alert-heading">Your lost items</h2>
        <p>
          You currently have{' '}
          <span className={`badge badge-pill badge-dark`}>{totalItems}</span>{' '}
          active reported lost items.
        </p>
        <div className="mb-3">{renderList()}</div>
        <p>These are your most recently active lost items.</p>
        <p className="mb-1">
          <Link
            to="/market?searchfor=&by=lost&page=1&pagesize=24"
