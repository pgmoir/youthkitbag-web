...
const mapStateToProps = (state) => ({
  totalItems: state.kitbag.market.founds.totalItems,
  items: state.kitbag.market.founds.items,
});
...
const KitbagMarketFoundAnnouncement = ({
  group,
  items,
  totalItems,
  fetchKitbagMarketItems,
  ...
  useEffect(() => {
    fetchKitbagMarketItems('found', 5);
  }, [fetchKitbagMarketItems]);
  ...
  return (
    <div className="card border-0">
      <div className="alert alert-danger mb-0" role="alert">
        <h2 className="alert-heading">Your found items</h2>
        <p>
          You currently have{' '}
          <span className={`badge badge-pill badge-dark`}>{totalItems}</span>{' '}
          active reported found items.
        </p>
